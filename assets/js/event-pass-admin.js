// event-pass-admin.js
// Event Pass Management Admin Dashboard - Complete Logic
...
function showToast(message, type) {
    ...
    toastIcon.innerText = type === 'success' ? '✅' : (type === 'error' ? '❌' : '⚠️');
            ...
            const eventsList = data.map(supEvent => {
                ...
                if (supEvent.description) {
                    ...
                    if (desc.includes('EarlyBird')) {
                        ...
                        const ebCapMatch = desc.match(/EarlyBird R\d+\/(\d+)/);
                    ...
                    if (desc.includes('General')) {
                        ...
                        const genCapMatch = desc.match(/General R\d+\/(\d+)/);
                        if (genCapMatch) ticketTypes.general.capacity = parseInt(genCapMatch[1]);
                    }
                    if (desc.includes('VIP')) {
                        const vipMatch = desc.match(/VIP R(\d+)/);
                        if (vipMatch) ticketTypes.vip.price = parseInt(vipMatch[1]);
                        const vipCapMatch = desc.match(/VIP R\d+\/(\d+)/);
                        if (vipCapMatch) ticketTypes.vip.capacity = parseInt(vipCapMatch[1]);
                    }
                }
...
function loadRefunds() {
    ...
    refundRequests = stored ? JSON.parse(stored) : [];
}
...
function loadActivityLog() {
    ...
    staffActivity = stored ? JSON.parse(stored) : [];
}

function logStaffActivity(action, details, targetId) {
    const activity = {
        id: Date.now(),
        action,
        details,
    ...
    staffActivity.unshift(activity);
    if (staffActivity.length > 200) staffActivity = staffActivity.slice(0, 200);
    localStorage.setItem(ACTIVITY_KEY, JSON.stringify(staffActivity));
}
...
function renderAccountList() {
    ...
    const filtered = accounts.filter(a => a.id.toLowerCase().includes(searchTerm) || (a.name && a.name.toLowerCase().includes(searchTerm)));
    ...
    container.innerHTML = filtered.map(acc => `
        <div class="account-item ${selectedAccountId === acc.id ? 'selected' : ''}" onclick="window.selectAccount('${acc.id}')">
            <div class="account-info">
                <div class="account-id">${escapeHtml(acc.id)}${acc.id === YOUR_PHONE ? '<span class="your-account-badge"><i class="fas fa-star"></i> YOU</span>' : ''}</div>
                <div class="account-name">${escapeHtml(acc.name || acc.id)}</div>
            </div>
            <div>
                <div class="account-balance">R${acc.balance.toFixed(2)}</div>
...
function renderAccountTransactions() {
    ...
    container.innerHTML = txs.slice(0, 8).map(tx => `
        <div class="transaction-item">
            <div class="transaction-left">
                <div class="transaction-icon"><i class="fas ${tx.amount > 0 ? 'fa-arrow-up' : 'fa-arrow-down'}"></i></div>
                <div class="transaction-details">
                    <div class="transaction-type">${escapeHtml(tx.desc)}</div>
                    <div class="transaction-time">${tx.time || tx.date}</div>
                </div>
            </div>
            <div class="transaction-amount ${tx.amount > 0 ? 'amount-positive' : 'amount-negative'}">${tx.amount > 0 ? '+' : ''}R${Math.abs(tx.amount).toFixed(2)}</div>
        </div>
    `).join('');
}
    ...
    container.innerHTML = refundRequests.map(req => `
        <div class="refund-item">
            <div class="refund-info">
                <div class="refund-id">#${req.id}</div>
                <div class="refund-details">${escapeHtml(req.userPhone || req.userId)} • ${req.reason || 'No reason'}</div>
            </div>
            <div>
                <div class="refund-amount">R${(req.amount || 0).toFixed(2)}</div>
                <div class="refund-status ${req.status === 'pending' ? 'status-pending' : req.status === 'approved' ? 'status-approved' : 'status-rejected'}">${req.status?.toUpperCase() || 'PENDING'}</div>
                ${req.status === 'pending' ? `<div style="display:flex; gap:5px; margin-top:8px;"><button class="action-btn success" style="padding:4px 10px; font-size:0.6rem;" onclick="window.approveRefund(${req.id})">Approve</button><button class="action-btn danger" style="padding:4px 10px; font-size:0.6rem;" onclick="window.rejectRefund(${req.id})">Reject</button></div>` : ''}
            </div>
        </div>
    `).join('');
}
...
function updateTicketStats() {
    ...
    for (const event of events) {
        for (const type in event.ticketTypes) {
    ...
    container.innerHTML = `
        <div class="event-card-full">
            <div class="event-banner-full">
                <span class="event-status-badge-full ${statusClass}">${event.status === 'active' ? '🔥 LIVE' : event.status.toUpperCase()}</span>
            </div>
            <div class="event-content-full">
                ...
                <div class="ticket-types-full">
                    ${event.ticketTypes.earlyBird ? `<div class="ticket-row-full"><span class="ticket-label-full">🎟️ Early Bird</span><span class="ticket-price-full">R${event.ticketTypes.earlyBird.price}</span><span class="ticket-sold-full">${event.ticketTypes.earlyBird.sold} sold</span></div>` : ''}
                    ${event.ticketTypes.general ? `<div class="ticket-row-full"><span class="ticket-label-full">👥 General Admission</span><span class="ticket-price-full">R${event.ticketTypes.general.price}</span><span class="ticket-sold-full">${event.ticketTypes.general.sold} sold</span></div>` : ''}
                    ${event.ticketTypes.vip ? `<div class="ticket-row-full"><span class="ticket-label-full">👑 VIP Experience</span><span class="ticket-price-full">R${event.ticketTypes.vip.price}</span><span class="ticket-sold-full">${event.ticketTypes.vip.sold} sold</span></div>` : ''}
                </div>
                <div class="stats-row-full">
                    <div class="stat-block-full"><div class="stat-number-full">${totalSold}</div><div class="stat-label-full">Sold</div></div>
                    <div class="stat-block-full"><div class="stat-number-full">${totalCap - totalSold}</div><div class="stat-label-full">Remaining</div></div>
                    <div class="stat-block-full"><div class="stat-number-full">${percent}%</div><div class="stat-label-full">Capacity</div></div>
                </div>
                <div class="event-actions-full">
                    <button class="action-btn-full view" onclick="window.viewEventDetails('${event.id}')"><i class="fas fa-eye"></i> View</button>
                    <button class="action-btn-full edit" onclick="window.openEditTicketModalById('${event.id}')"><i class="fas fa-edit"></i> Edit</button>
                    <button class="action-btn-full delete" onclick="window.deleteSelectedEventById('${event.id}')"><i class="fas fa-trash"></i> Delete</button>
                    <button class="action-btn-full duplicate" onclick="window.duplicateEvent('${event.id}')"><i class="fas fa-copy"></i> Duplicate</button>
                </div>
            </div>
        </div>
    `;
...
function renderTicketInventory() {
    ...
    container.innerHTML = types.map(t => {
        ...
        return `
            <div class="transaction-item">
                <div class="transaction-left">
                    <div class="transaction-icon"><i class="fas fa-ticket-alt"></i></div>
                    <div class="transaction-details">
                        <div class="transaction-type">${t.label}</div>
                        <div class="transaction-time">Price: R${data.price} | Cap: ${data.capacity}</div>
                    </div>
                </div>
                <div class="transaction-amount amount-positive">Sold: ${data.sold}<br><span style="font-size:0.6rem;">Left: ${remaining}</span></div>
            </div>
        `;
    }).join('');
}
...
function renderEventPurchases() {
    ...
    container.innerHTML = eventTickets.map(t => `
        <div class="transaction-item">
            <div class="transaction-left">
                <div class="transaction-icon"><i class="fas fa-user"></i></div>
                <div class="transaction-details">
                    <div class="transaction-type">${escapeHtml(t.ticketType)}</div>
                    <div class="transaction-time">ID: ${t.ticketId?.slice(-12)}</div>
                </div>
            </div>
            <div class="transaction-amount amount-positive">R${t.price?.toFixed(2) || 0}</div>
        </div>
    `).join('');
}
...
async function confirmAddEvent() {
    ...
    const description = `Tickets: EarlyBird R${earlyBirdPrice}/${earlyBirdCap} | General R${generalPrice}/${generalCap} | VIP R${vipPrice}/${vipCap}`;
...
function renderActivityLog() {
    ...
    container.innerHTML = staffActivity.slice(0, 50).map(act => `
        <div class="activity-item">
            <div class="transaction-left">
                <div class="transaction-icon"><i class="fas fa-user-cog"></i></div>
                <div class="transaction-details">
                    <div class="transaction-type">${act.action.replace(/_/g, ' ')}</div>
                    <div class="transaction-time">${act.time} • ${act.date}</div>
                    <div style="font-size:0.65rem;">${escapeHtml(act.details)}</div>
                </div>
            </div>
        </div>
    `).join('');
...
function renderGlobalActivity() {
    ...
    for (const accId in transactionsMap) {
        const txs = transactionsMap[accId];
        txs.forEach(tx => {
            allTx.push({
                ...tx,
                accountId: accId,
    ...
    container.innerHTML = recent.map(tx => `
        <div class="transaction-item">
            <div class="transaction-left">
                ...
                <div class="transaction-details">
                    <div class="transaction-type">${escapeHtml(tx.desc)}</div>
                    <div class="transaction-time">${tx.accountName}</div>
                </div>
            </div>
            <div class="transaction-amount ${tx.amount > 0 ? 'amount-positive' : 'amount-negative'}">${tx.amount > 0 ? '+' : ''}R${Math.abs(tx.amount).toFixed(2)}</div>
        </div>
...
function switchTab(tab) {
    ...
    btns.forEach((btn) => {
        const btnTab = btn.innerText.toLowerCase().includes('passport') ? 'wallet' : btn.innerText.toLowerCase().includes('ticket') ? 'tickets' : 'activity';
