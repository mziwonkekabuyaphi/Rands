...
- 
- Repo layout this file assumes:
- /login.html            (root)
- /config/supabase.js    (exports `supabase`)
- /assets/js/login.js    (this file)
- 
- Real Supabase auth · role-based redirect · no global variables · async/await
  */
...
// Remove then re-add .visible to retrigger the CSS animation on repeat errors
...
// Success — fetch role then navigate; page leaves so no need to reset loading state
...
async function init() {
await checkExistingSession(); // redirect silently if already logged in
